...
qqnorm(cox_residuals2)
qqline(cox_residuals2)

cox.zph_result2 <- cox.zph(cox_model2)
cox.zph_result2
plot(cox.zph_result2)
```

This is using the interaction term Gender:Cancer_Group to the model
```{r, warning=FALSE, echo=TRUE}
cox_model_interaction <- coxph(Surv(Survival.months) ~ Gender + Cancer_Group + Gender:Cancer_Group, data = filtered_data)
cox_summary_interaction <- summary(cox_model_interaction)
table_data_interaction <- cox_summary_interaction$coefficients[, c("coef", "exp(coef)", "Pr(>|z|)")]
kable(table_data_interaction, 
      caption = "Cox Proportional Hazard Model Results on Deceased Patients with Interaction Term",
      format = "html", 
      align = "c", 
      col.names = c("Coefficient", "Hazard Ratio", "p-value")) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
```{r, warning=FALSE, echo=TRUE}
cox_residuals_interaction <- residuals(cox_model_interaction, type = "martingale")
summary(cox_residuals_interaction)
qqnorm(cox_residuals_interaction)
qqline(cox_residuals_interaction)

cox.zph_result_interaction <- cox.zph(cox_model_interaction)
print(cox.zph_result_interaction)
plot(cox.zph_result_interaction)
```
...
qqnorm(cox_residuals3)
qqline(cox_residuals3)

cox.zph_result3 <- cox.zph(cox_model3)
cox.zph_result3
plot(cox.zph_result3)
```
Time Dependent Covariates..

```{r, warning=FALSE, echo=TRUE}
cox_model_time_dependent <- coxph(Surv(Survival.months, survived) ~ Age + Gender + tt(Race_Category), data = df)
cox_summary_time_dependent <- summary(cox_model_time_dependent)
table_data_time_dependent <- cox_summary_time_dependent$coefficients[, c("coef", "exp(coef)", "Pr(>|z|)")]
kable(table_data_time_dependent, 
      caption = "Cox Proportional Hazard Model with Time-Dependent Covariates",
      format = "html", 
      align = "c", 
      col.names = c("Coefficient", "Hazard Ratio", "p-value")) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
```{r, warning=FALSE, echo=TRUE}
cox_model_time_dependent_res <- residuals(cox_model_time_dependent, type = "martingale")
summary(cox_model_time_dependent_res)
qqnorm(cox_model_time_dependent_res)
qqline(cox_model_time_dependent_res)
...
```{r, warning=FALSE, echo=TRUE}
cox_model_piecewise <- coxph(Surv(Survival.months, survived) ~ Age + Gender + Race_Category, data = df, tt = list(Age))
cox_summary_piecewise <- summary(cox_model_piecewise)
table_data_piecewise <- cox_summary_piecewise$coefficients[, c("coef", "exp(coef)", "Pr(>|z|)")]
kable(table_data_piecewise, 
      caption = "Piecewise Cox Proportional Hazard Model",
      format = "html", 
      align = "c", 
      col.names = c("Coefficient", "Hazard Ratio", "p-value")) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
...
```{r, warning=FALSE, echo=TRUE}
cox_model_piece_res <- residuals(cox_model_piecewise, type = "martingale")
summary(cox_model_piece_res)
qqnorm(cox_model_piece_res)
qqline(cox_model_piece_res)

cox.zph_result_piecewise <- cox.zph(cox_model_piecewise)
print(cox.zph_result_piecewise)
plot(cox.zph_result_piecewise)
```
