...
In our earlier data preparation steps, we took the time to get our dataset in shape. As part of our check, we assessed for any missing values. Interestingly, we noticed that the 'Survival.months' column had some entries labeled as 'Unknown'.To keep things reliable, we decided to drop these entries, ensuring our dataset is accurate for the model performance. 
...
Below we check if the proportional hazards assumption holds true for each factor in the Cox model. This assumption is extremely important in survival analysis because it suggests that the influence of factors stays the same over time. The proportional hazards assumption implies that the effect of a variable (like gender or age) on the hazard of an event remains constant or proportional over different durations or time periods.
```{r, warning=FALSE, echo=TRUE}
...
Due to our proportional hazards assumptions being violated, we decided to use stratification. Stratification is used to handle the influence of variables that violate the proportional hazards assumption. When certain variables are suspected to have different baseline hazard functions, which means they violate the proportional hazards assumption, we use the strata function in R to stratify the model by those variables. This allows each stratum to  have its own baseline hazard function, allowing for varying hazard ratios across strata. For this particular part, we decided to strata sex and age in order to see how our model will perform. 
...
kable(table_data, 
  ...
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
After narrowing down our focus to Thyroid Cancer cases, the Cox Proportional Hazard model reveals various insights. Black race category has a coefficient of -0.0724061, indicating a hazard ratio of 0.9301531. This suggests a 6.99% decrease in the hazard of Thyroid Cancer compared to other racial categories. Being male has a coefficient of 0.0351059, resulting in a hazard ratio of 1.0357294. This indicates a 3.57% increase in the hazard of the event for males compared to females among thyroid cancer patients. For every one-unit increase in age, there's a hazard ratio of 1.0012238, suggesting a 0.12% increase in the hazard of the event per unit increase in age among thyroid cancer patients.
...
#### Time Dependent Analysis with Thyroid Cancer

We decided to implement a time dependent covariates as stated above. This approach enables us to examine how the hazard ratios categories change over time, providing a better understanding of their impact on survival outcome. The tt function is used from R to create time-transformed variables for selected covariates. For the code below, we made Race_Category a tt variable.
...
kable(table_data, 
  ...
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
The time-dependent Cox Proportional Hazards model, specifically focusing on Thyroid Cancer, reveals various findings. Among race categories, while Black race shows a marginal increase 1.57% in the hazard compared to the reference group, Other/Miscellaneous, Pacific Islander, and notably, White race exhibit significantly higher hazard ratios, with White race displaying a hazard ratio of 14.03. Additionally, being male corresponds to a 3.59% higher hazard compared to females among Thyroid Cancer patients. Age shows a small yet significant impact, with a 0.12% increase in the hazard per unit increase in age. Notably, the time-dependent interaction term (tt(Race_Category)) suggests a varying impact of race categories on the hazard over time, revealing a decrease in hazard ratio for a specified race category with each unit increase in time compared to the reference time point. 
...
```
The cox.zph() function does not support time-transformed terms. Nevertheless, we can visually inspect the Schoenfeld residuals through plots to identify trends or patterns that might indicate a violation of the proportional hazards assumption. Our plots suggests that the proportional hazards assumption is not met. We will explore an alternative approach by incorporating interactions in our model.

#### Interaction Analysis with Thyroid Cancer

In survival analysis, an interaction term is often used to assess whether the effect of one variable on the outcome is modified by the presence of another variable. It allows us to explore whether the combined influence of two variables is greater than the sum of their individual effects.For the code below, we used Race_Category as an interaction term for Thyroid Cancer.
...
plot(cox.zph_result_filtered)
```
The GLOBAL test for the proportional hazards assumption combines all the variables and interactions. With a p-value of 0.00610, there’s evidence suggesting a violation of the proportional hazards assumption in the model among Thyroid Cancer cases. These results indicate that while some individual variables or interactions might not violate the assumption significantly(like Sex and Age), the model as a whole shows violation of proportional hazards assumption among thyroid cancer patients. To further test this out we decided to use stratification in our next analysis. 

#### Filtered Analysis with Stratification on Thyroid Cancer

For the code below, we use strata for Race_Category and kept sex and age the same. 
...
#### Filtered Analysis on Cancer and Race with Thyroid Cancer

Below, we create a new variable called "Binary_Race" where we separate it into a binary variable" Black or other. This helps simplify the analysis and helps us dig deeper to find a model that does not violate the model.  
...
plot(cox.zph_result_filtered)
```
Sex shows no evidence of violating the proportional hazards assumption, both Race_Category and especially age does. The global test confirms a significant violation, suggesting that the model doesn't satisfy the assumption of constant hazard ratios over time for Renal Cancer cases.
...
#### Stratification Analysis on Renal Cancer 

For this portion of the code, we decided to strata(Age).
...
plot(cox.zph_result_filtered)
```
For sex we have a non-significant p-value of .3592, indicating no strong evidence of a violation. However, for race_category we do violate our proportional hazards assumptions. This indicates that the effect of race on survival time changes over time. Because of this low p-value we decided to use interaction terms as well to see how our model improves. 

#### Interaction Analysis with Renal Cancer 

In the code below we used interaction terms and kept the strata of age. We used Race_Category as a interaction term. 
...
plot(cox.zph_result_filtered)
```
Based on our p-values for the Schoenfield test, we are still violating the proportional hazards. Thus, we decided to do a similar approach as last time since Race_Category seems to be the issue. We decided to do the Binary_Race. 

#### Filtering Analysis on Race with Renal Cancer 

For the code below, we created the Binary_Race variable similar to how we did in with Thyroid Cancer. We made Binary_Race an interaction term as well and strata the variable age.
...
kable(table_data, 
  ...
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
The coefficient for Binary_RaceOther is 0.0125754, resulting in a hazard ratio of 1.0126548. This implies an increase of 1.27% in the hazard of the event for individuals categorized as Other compared to those categorized as Black with Renal Cancer, as the p-value is 0.5541234. The coefficient for being male is 0.0164791, resulting in a hazard ratio of 1.0166157. This implies an increase of 1.65% in the hazard of the event for males compared to females with Renal Cancer, as the p-value is 0.5131259. The coefficient for the interaction term is -0.0214101, resulting in a hazard ratio of 0.9788175. This implies a decrease of 2.12% in the hazard of the event for individuals categorized as Other compared to those categorized as Black specifically among males, as the p-value is 0.4257748.
...
plot(cox.zph_result_filtered)
```
The Schoenfeld test shows that none of the variables significantly violate the proportional hazards assumption individually. The GLOBAL test reinforces this, suggesting that, collectively, the variables do not provide strong evidence against the proportional hazards assumption. By editing the Race_Category we found that our model improved overall similar to what was done in our Thyroid Cancer analysis. 

## Conclusion

In our analysis, we've successfully pinpointed key factors influencing cancer survival, shedding light on the intricate dynamics shaping patient outcomes. Our exploration has shown the multifaceted nature of survival dynamics, revealing complex relationships among factors and survival duration. Identifying the violations in the proportional hazards assumption prompted further investigation. To tackle this, we strategically employed advanced methods, including stratification, interaction terms, and time-dependent covariates. We also filtered data on specific cancer types, such as Thyroid and Renal Cancer, crucial for capturing evolving predictors over time. Our findings not only highlight critical variables in cancer survival but stress the importance of dynamic modeling techniques for unraveling intricacies in survival analysis. Focusing our model on specific cancer types allowed for a clearer understanding, minimizing confounding variables and enhancing analysis precision. Challenges arose when filtering on high-mortality cancer types, like pancreatic, leading to inflated significance levels due to limited variability. Throughout our analyses, we assessed proportional hazards assumptions and encountered deviations especially with variables related to Race_category. Although we were able to mitigate this, it still proved to be a problem. Moving forward, these variable analyses should be taken into consideration since it has limitations in modeling survival dynamics in different cancer subgroups. Overall, through our modeling we were able to have a better understanding on cancer survival and their relationships with the variables we tested. 
