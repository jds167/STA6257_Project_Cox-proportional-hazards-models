...
The model assumes that the effect of the covariates on the hazard rate is proportional over time thus the name 'proportional hazards'. The hazard ratio between two individuals remains constant over time if one individual's covariate values are a constant factor of the other's. In addition, the model assumes that the likelihood of being censored is not related to the unobserved event times. If censoring is informative, it can lead to bias in the model. The Cox Proportional Hazard model assumes that there are no interactions between covariates. 

The key advantage of the Cox model is that it doesn't make specific assumptions about the shape of the hazard function. This makes it useful for various applications as seen in the introduction. It relies on partial likelihood estimation to estimate the coefficients $β₁, β₂, ..., βₖ. Another limitation to be cautious of is that the model assumes that log hazard is linear in covariates. When this assumption is not met, it can cause the model to not fit the data well. Thus, it's important to transform variables. It is important to verify the proportional hazards assumption, consider potential confounders, and address issues such as censoring and collinearity in the data. 
