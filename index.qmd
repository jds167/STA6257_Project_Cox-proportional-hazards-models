...
Seer_updated %>%
  ...
  labs(title = "Top 10 Categories of Death for Cancer",
       x = "Site Recorded",
       y = "Count") +
  theme_minimal() +  
...
plot(cox.zph_result_filtered)
```
For sex we have a non-signficiant p-value of .3592, indicating no strong evidence of a departure from proportional hazards for the variable. However, for race_category we have a signficiant departure from proportional hazards. This indicates that the effect of race on survival time changes over time. Because of this low p-value we decided to use interaction terms as well to see how our model improves. 
